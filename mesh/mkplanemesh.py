...
def mkmesh_plane(porder, ndim, meshfile):

    mesh_save = meshfile + '_processed'
    logging.info(mesh_save)
    if os.path.exists(mesh_save):
        # Reading mesh from disk
        logging.info('/Mesh: Reading processed mesh from file...')
        with open(mesh_save, 'rb') as file:
            mesh = pickle.load(file)
        return mesh
        
    logging.info('/Mesh: processing mesh...')
    ...
    logging.info('/Mesh: make t2f, f')
    mesh['f'], mesh['t2f'] = mkt2f_new(mesh['t'], 3)

    logging.info('/Mesh: master nodes')
    mesh['plocal'], mesh['tlocal'], _, _, _, _, _ = masternodes(porder, 3)

    # set boundary numbers
    logging.info('/Mesh: assigning BC flags')
    mesh = assign_BC_flags(mesh)

    # create dg nodes
    logging.info('/Mesh: creating high order nodes')
...
if __name__ == '__main__':
    porder = 3
    mkmesh_plane(porder, 3, '../../data/797_coarse')