...
% plot the solution field on the plane c(1)*x + c(2)*y + c(3)*z + c(4) = 0

if mesh.nd ~= 3
    error('slice plot is only for 3D field.');
end
if (mesh.porder == 0) || (size(udg,1) == 1)
    error('porder must be greater than 0.');
end
if length(c)~=4
    error('Input vector c must have 4 entries.')
end
...
% elements intersects the hyperplane
...
udg = udg(:,inde);
pdg = permute(mesh.dgnodes(:,:,inde),[1 3 2]);
...
% case 1
in1 = find( sum(abs(d1)+abs(d2)+abs(d3))<1e-12 );
ne1 = length(in1);
pp{1} = pdg(1:nd,in1,:);
...
% case 2
in2 = find( sum(abs(d1)+abs(d2)+abs(d4))<1e-12 );
ne2 = length(in2);
pp{2} = pdg([1 2 4],in2,:);
...
% case 3
in3 = find( sum(abs(d1)+abs(d3)+abs(d4))<1e-12 );
ne3 = length(in3);
pp{3} = pdg([1 3 4],in3,:);
...
% case 4
in4 = find( sum(abs(d2)+abs(d3)+abs(d4))<1e-12 );
ne4 = length(in4);
pp{4} = pdg([2 3 4],in4,:);
