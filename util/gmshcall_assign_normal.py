...
def gmshcall(filename, ndim, elemtype, elemtype_face=None):
    ...
    if elemtype_face:
        ...
        for key in phys_grp_dict:       # Key is the surface # (1-indexed)
            ...
            if col_idx is not None:     # If the surface is planar
        ...
        for axis in bdry_surf_idx:
            ...
            nodes = phys_grp_dict[min_key]['nodes']
            surf_unique_pts_and_normals = np.zeros((nodes.shape[0], 4))
            surf_unique_pts_and_normals[:, 0] = nodes
            surf_unique_pts_and_normals[:, 1:] = neg_normal_vec
            phys_grp_dict[min_key]['nodes'] = surf_unique_pts_and_normals
            ...
            # Create the normal array and set the normals to -1
            nodes = phys_grp_dict[max_key]['nodes']
            surf_unique_pts_and_normals = np.zeros((nodes.shape[0], 4))
            surf_unique_pts_and_normals[:, 0] = nodes
            surf_unique_pts_and_normals[:, 1:] = -neg_normal_vec    # Don't have to recreate the normal vector from last time, just flip it to make it positive
            phys_grp_dict[max_key]['nodes'] = surf_unique_pts_and_normals
